...
set_plot_params(fontfamily='serif', fontstyle='normal', fontvariant='normal',
                fontweight='normal', fontstretch='normal', fontsize=smallFontSize, useTex='True')

Home = os.environ['HOME']

keplerPath = os.path.join(Home, "Documents", "MNRASPaper3", "Kepler")
outPath = os.path.join(Home, "Documents", "MNRASPaper3", "Zw229-15Results", "060120170937")
...
def get_greens_func(t, percentile, ar_roots):
    ...
    for i in xrange(num_t):
        temp_gfunc[:] = (np.exp(ar_roots[:, 0]*t[i])-np.exp(ar_roots[:, 1]*t[i])) / \
            (ar_roots[:, 0]-ar_roots[:, 1])
        gfunc_mid[i] = complex(np.median(temp_gfunc[:].real), np.median(temp_gfunc[:].imag))
        gfunc_high[i] = complex(np.percentile(temp_gfunc[:].real, upper),
                                np.percentile(temp_gfunc[:].imag, upper))
        gfunc_low[i] = complex(np.percentile(temp_gfunc[:].real, lower),
                               np.percentile(temp_gfunc[:].imag, lower))
...
def get_greens_func_max(t_maxes, ar_roots):
    ...
    val[:] = (np.exp(ar_roots[:, 0].real*t_maxes[:]) - np.exp(ar_roots[:, 1]*t_maxes[:])) / \
        (ar_roots[:, 0] - ar_roots[:, 1])
    return val
...
def get_dist_psd(f, percentile, ma_coefs):
    ...
    for i in xrange(num_f):
        ...
        temp_psd[:] = OneOverTwoPi*(poly_coefs[:, 1]*poly_coefs[:, 1] *
                                    evalFreq + poly_coefs[:, 0]*poly_coefs[:, 0])
...
data = np.loadtxt(os.path.join(keplerPath, keplerObj, keplerObj+'-calibrated.dat'), skiprows=2)
t = list()
y = list()
yerr = list()
cadNo = list()
...
outputSample = open(os.path.join(outPath, 'Sample.pkl'), 'rb')
...
if ((makeFig1 is True) or (makeFig2 is True) or (makeFig4 is True) or (makeAllFigs is True)):
    if ((makeFig1 is True) or (makeAllFigs is True)):
    ...
    psd_low, psd_high, psd_mid, frequencies = sample.plot_power_spectrum(
        percentile=95.0, nsamples=numSamples/ptFactor, doShow=False)
...
if ((makeFig2 is True) or (makeAllFigs is True)):
...
if ((makeFig3 is True) or (makeFig1 is True) or (makeAllFigs is True)):
...
if ((makeFig4 is True) or (makeAllFigs is True)):
...
if ((makeFig5 is True) or (makeFig2 is True) or (makeAllFigs is True)):
    ...
    for i in xrange(t_maxes.shape[0]):
        tau_solution[i] = fsolve(gfunc_etime, tau_initial_guess, args=(
            ar_roots[i, 0].real, ar_roots[i, 1].real, gfunc_efold[i]))
    percentile = 95.0
    lower = (100.0 - percentile)/2.0  # lower and upper intervals for credible region
    upper = 100.0 - lower
...
# if ((makeFig7 is True) or (makeAllFigs is True)):
#     t_turns = 2.0*m.pi*poly_coefs[:, 1]/poly_coefs[:, 0]
...
if ((makeFig8 is True) or ((makeFig1 is True) and (plotMockLC is True)) or (makeAllFigs is True)):
    ...
    y_sim_no_noise = mu_sim + \
        cmcmc.carma_process(t, sigmay_sim*sigmay_sim, ar_roots_sim, ma_coefs=ma_coefs_sim)
...
if ((makeFig1 is True) or (makeAllFigs is True)):
    ...
    ax1.errorbar(0, 0, yerr=0, fmt='.', capsize=0, color='#d95f02',
                 markeredgecolor='none', label=r'observed Zw 229-15 light curve')
    ax1.errorbar(t, y, yerr=yerr, fmt='.', capsize=0, color='#d95f02', markeredgecolor='none', zorder=10)
    if (plotMockLC is True):
        ax1.errorbar(0, 0, yerr=0, fmt='.', capsize=0, color='#66a61e',
                     markeredgecolor='none', label=r'simulated Zw 229-15 light curve')
        ax1.errorbar(t, y_sim, yerr=yerr_sim, fmt='.', capsize=0,
                     color='#66a61e', markeredgecolor='none', zorder=-5)
    ...
    ax1.annotate(r'C-ARMA(2,1)', xy=(0.135, 0.1), xycoords='axes fraction',
                 textcoords='axes fraction', ha='center', va='center', multialignment='center', fontsize=32)
    ax1.annotate(r'AICc: -2333427.16925', xy=(0.135, 0.05), xycoords='axes fraction',
                 textcoords='axes fraction', ha='center', va='center', multialignment='center', fontsize=24)
    ...
    ax1.hlines(y=nLevel, xmin=np.min(frequencies), xmax=np.max(frequencies), colors='#e41a1c',
               linestyles='solid', linewidths=2, label=r'Noise level', zorder=10)
    ...
    plt.tight_layout()
    if (plotMockLC is True):
        figure1JPG = os.path.join(outPath, 'Zw229-15_mock+LC.jpg')
        plt.savefig(figure1JPG, dpi=300)
        if (makePDF is True):
            figure1PDF = os.path.join(outPath, 'Zw229-15_mock+LC.pdf')
            plt.savefig(figure1PDF, dpi=300)
        if (makeEPS is True):
            figure1EPS = os.path.join(outPath, 'Zw229-15_mock+LC.eps')
            plt.savefig(figure1EPS, dpi=300)
    else:
        figure1JPG = os.path.join(outPath, 'Zw229-15_LC.jpg')
        plt.savefig(figure1JPG, dpi=300)
        if (makePDF is True):
            figure1PDF = os.path.join(outPath, 'Zw229-15_LC.pdf')
            plt.savefig(figure1PDF, dpi=300)
        if (makeEPS is True):
            figure1EPS = os.path.join(outPath, 'Zw229-15_LC.eps')
            plt.savefig(figure1EPS, dpi=300)
...
if ((makeFig2 is True) or (makeAllFigs is True)):
    ...
    ax2.vlines(x=t_max_med, ymin=m.pow(10.0, -11.0), ymax=m.pow(10.0, 1.0), colors='#d95f02',
               linestyles='--', linewidths=2, label=r'$t_{\mathrm{max}}$', zorder=10)
    ax2.vlines(x=tau_solution_med, ymin=m.pow(10.0, -11.0), ymax=m.pow(10.0, 1.0), colors='#d95f02',
               linestyles='-.', linewidths=2, label=r'$t_{\mathrm{e-fold}}$', zorder=10)
    ...
    plt.tight_layout()
    figure2JPG = os.path.join(outPath, 'Zw229-15_GF.jpg')
    plt.savefig(figure2JPG, dpi=300)
    if (makePDF is True):
        figure2PDF = os.path.join(outPath, 'Zw229-15_GF.pdf')
        plt.savefig(figure2PDF, dpi=300)
    if (makeEPS is True):
        figure2EPS = os.path.join(outPath, 'Zw229-15_GF.eps')
    ...
    scatPlot = axm1.scatter(t_maxes[:].real, tau_solution[:], c=best_loglike -
                            loglike, marker='.', cmap=colormap.gist_rainbow_r, linewidth=0, zorder=20)
    ...
    cBar = plt.colorbar(scatPlot, ax=axm3, orientation='horizontal',
                        ticks=cBarTicks, format=r'$\scriptscriptstyle %2.1f$')
    ...
    axm1.annotate(r'$t_{\mathrm{max}} = %3.1f^{+%2.1f}_{-%2.1f}$ (d)'%(t_max_med, t_max_high-t_max_med, t_max_med-t_max_low), xy=(
        0.7, 0.85), xycoords='axes fraction', textcoords='axes fraction', ha='center', va='center', multialignment='center', fontsize=24)
    axm1.annotate(r'$t_{\mathrm{e-fold}} = %2.1f^{+%2.1f}_{-%2.1f}$ (d)'%(tau_solution_med, tau_solution_high-tau_solution_med, tau_solution_med-tau_solution_low),
                  xy=(0.71, 0.75), xycoords='axes fraction', textcoords='axes fraction', ha='center', va='center', multialignment='center', fontsize=24)

    plt.tight_layout()

    figurem2JPG = os.path.join(outPath, 'Zw229-15_GFm.jpg')
    plt.savefig(figurem2JPG, dpi=300)
    if (makePDF is True):
        figurem2PDF = os.path.join(outPath, 'Zw229-15_GFm.pdf')
        plt.savefig(figurem2PDF, dpi=300)
    if (makeEPS is True):
        figurem2EPS = os.path.join(outPath, 'Zw229-15_GFm.eps')
        plt.savefig(figurem2EPS, dpi=300)
...
if ((makeFig3 is True) or (makeAllFigs is True)):
    ...
    scatPlot = ax1.scatter(ar_roots[:, 0].real, ar_roots[:, 1].real, c=best_loglike -
    ...
    ax1.annotate(r'$T_{\mathrm{dHO}} = %3.1f^{+%2.1f}_{-%2.1f}$ (d)'%(tscale_mid, tscale_high-tscale_mid, tscale_mid-tscale_low), xy=(
        0.7125, 0.25), xycoords='axes fraction', textcoords='axes fraction', ha='center', va='center', multialignment='center', fontsize=24)
    ax1.annotate(r'$\zeta_{\mathrm{dHO}} = %3.2f^{+%3.2f}_{-%3.2f}$'%(zeta_mid, zeta_high-zeta_mid, zeta_mid-zeta_low), xy=(
...
if ((makeFig4 is True) or (makeAllFigs is True)):
    ...
    ax1.loglog(distfreqs, dist_psd_mid, '-', color='#666666',
               label=r'median $S_{uu}$', zorder=5, subsx=[], subsy=[])
    ax1.fill_between(distfreqs, dist_psd_low, dist_psd_high, color='#b3b3b3', zorder=0)
    ax1.vlines(x=np.median(t_turns[:].real), ymin=m.pow(10.0, -17.25), ymax=m.pow(10.0, -13),
               colors='#d95f02', linestyles='--', linewidths=2, label=r'$t_{\mathrm{turn}}$', zorder=10)
    ...
    if scatterMA is True:
        ...
        scatPlot = ax2.scatter(poly_coefs[:, 0], poly_coefs[:, 1], c=best_loglike -
                               loglike, marker='.', cmap=colormap.gist_rainbow_r, linewidth=0)
        ...
        for i in xrange(numCBarTicks):
            cBarTicks[i] = -3.0*i-1.5
        cBar = plt.colorbar(scatPlot, ax=ax3, orientation='horizontal',
                            ticks=cBarTicks, format=r'$\scriptscriptstyle %2.1f$')
        cBar.set_label(r'Relative Likelihood', fontsize=scriptFontSize)
    ...
    plt.tight_layout()
    figure4JPG = os.path.join(outPath, 'Zw229-15_distPSD.jpg')
    plt.savefig(figure4JPG, dpi=300)
    if (makePDF is True):
        figure4PDF = os.path.join(outPath, 'Zw229-15_distPSD.pdf')
        plt.savefig(figure4PDF, dpi=300)
    if (makeEPS is True):
        figure4EPS = os.path.join(outPath, 'Zw229-15_distPSD.eps')
...
if ((makeFig5 is True) or (makeAllFigs is True)):
    ...
    t_max_high = np.percentile(t_maxes[:].real, upper)
    t_max_low = np.percentile(t_maxes[:].real, lower)
    nums, bins, patches = ax1.hist(t_maxes[:].real, nBinsFig5, normed=0,
                                   edgecolor='#666666', facecolor='#b3b3b3')
    ...
    plt.tight_layout()
    figure5JPG = os.path.join(outPath, 'Zw229-15_tmax.jpg')
    plt.savefig(figure5JPG, dpi=300)
    if (makePDF is True):
        figure5PDF = os.path.join(outPath, 'Zw229-15_tmax.pdf')
...
if ((makeFig6 is True) or (makeAllFigs is True)):
    ...
    nums, bins, patches = ax1.hist(t_turns[:].real, nBinsFig6, normed=0,
                                   edgecolor='#666666', facecolor='#b3b3b3')
    ...
    plt.tight_layout()
    figure5JPG = os.path.join(outPath, 'Zw229-15_tturn.jpg')
    plt.savefig(figure5JPG, dpi=300)
    if (makePDF is True):
        figure5PDF = os.path.join(outPath, 'Zw229-15_tturn.pdf')
        plt.savefig(figure5PDF, dpi=300)
    if (makeEPS is True):
        figure5EPS = os.path.join(outPath, 'Zw229-15_tturn.eps')
        plt.savefig(figure5EPS, dpi=300)
...
if ((makeFig7 is True) or (makeAllFigs is True)):
    ...
    nums, bins, patches = ax1.hist(measerr_scale, nBinsFig7, normed=0,
                                   edgecolor='#666666', facecolor='#b3b3b3')
    ...
    plt.tight_layout()
    figure5JPG = os.path.join(outPath, 'Zw229-15_MErr.jpg')
    plt.savefig(figure5JPG, dpi=300)
    if (makePDF is True):
        figure5PDF = os.path.join(outPath, 'Zw229-15_MErr.pdf')
        plt.savefig(figure5PDF, dpi=300)
    if (makeEPS is True):
        figure5EPS = os.path.join(outPath, 'Zw229-15_MErr.eps')
        plt.savefig(figure5EPS, dpi=300)
...
if ((makeFig8 is True) or (makeAllFigs is True)):
    ...
    ax1.errorbar(0, 0, yerr=0, fmt='.', capsize=0, color='#666666',
                 markeredgecolor='none', label=r'simulated Zw 229-15 light curve')
    ax1.errorbar(t, y_sim, yerr=yerr_sim, fmt='.', capsize=0,
                 color='#666666', markeredgecolor='none', zorder=10)
    ...
    plt.tight_layout()
    figure8JPG = os.path.join(outPath, 'Zw229-15_mockLC.jpg')
    plt.savefig(figure8JPG, dpi=300)
    if (makePDF is True):
        figure8PDF = os.path.join(outPath, 'Zw229-15_mockLC.pdf')
        plt.savefig(figure8PDF, dpi=300)
    if (makeEPS is True):
        figure8EPS = os.path.join(outPath, 'Zw229-15_mockLC.eps')
...
if ((makeFig9 is True) or (makeAllFigs is True)):
