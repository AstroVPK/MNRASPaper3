...
Defining $k = 2q - i - j$ \& $l = 2p - m - n$ allows us to rewrite equation~\ref{eq:CARMAPSDForm1} as
...
\gamma_{\chi\chi}(T) = \int_{-\infty}^{\infty}\chi(t)\chi(t + T) dt = \sum_{k = 1}^{p} \frac{\beta(\rho_{k}) \beta(-\rho_{k})}{\alpha(\rho_{k})\alpha(-\rho_{k})}\mathrm{e}^{\rho_{k} T},
\end{equation}
where the $\rho$ are the roots of the autoregressive characteristic polynomial.
...
\mathbfss{D}_{i,j} = \int_{0}^{\delta t} \mathrm{e}^{(w_{i} + w_{j})\xi} \mathbfss{C}_{i,j} d\xi = \mathbfss{C}_{i,j} \frac{\mathrm{e}^{(w_{i} + w_{j})\delta t} - 1}{w_{i} + w_{j}}.
\end{equation}
Then we calculate
...
\mathbfss{D'}_{i,j} = \int_{0}^{\infty} \mathrm{e}^{(w_{i} + w_{j})\xi} \mathbfss{C}_{i,j} d\xi = \mathbfss{C}_{i,j} \frac{ - 1}{w_{i} + w_{j}}.
\end{equation}
and use it to compute
\begin{equation}\label{eq:Sigma}
\boldsymbol{\Sigma} = \mathbfss{v}\mathbfss{D}\mathbfss{v}^{\top},
\end{equation}
...
\section[Sampling]{Types of Sampling Patterns}\label{sec:Sampling}
...
We have implemented the C-ARMA analysis presented above in a fast, fully parallelized and vectorized software package, \textsc{libcarma}, in the \textsc{c++} programming language with \textsc{Python} language bindings for ease of use. The \textsc{libcarma} package may be obtained at \textit{https://github.com/AstroVPK/libcarma} and may be installed on Linux variants and Mac OSX using the install scripts provided.
...
\begin{figure}
	\includegraphics[width=\columnwidth]{images/Zw229-15_LC.jpg}
...
\begin{figure}
	\includegraphics[width=\columnwidth]{images/Zw229-15_GFunc.jpg}
...
\begin{figure}
	\includegraphics[width=\columnwidth]{images/Zw229-15_Times.jpg}
