...
\begin{abstract}
AGN exhibit rapid, high amplitude stochastic flux variability across the entire electromagnetic spectrum on timescales ranging from hours to years. The cause of this variability is poorly understood because of the complexity of the phenomenon being observed. We present a new method for using variability to (1) measure the time-scales on which flux perturbations evolve and (2) characterize the driving flux perturbations. We decompose the observed light curve fluctuations into stochastic impulses that drive an ordinary linear differential equation. The Green's function of the differential equation quantifies the time-domain evolution of perturbations to the emitted flux and hence probes dissipative physical processes in accretion flow. The PSD of the stochastic driving impulses characterizes the physical processes responsible for the flux perturbations. Applying our technique to the light curve of the \Kepler AGN Zw 229-15, we find that the observed variability behavior can be modeled as a damped harmonic oscillator perturbed by a colored noise process. The model PSD turns over on time-scale $385$~d. On shorter time-scales, the model PSD log slope varies between $2$ and $4$, explaining the behavior noted by previous studies. We recover and identify both the $5.6$~d and $67$~d timescales reported by previous work. In our model, these timescales represent the time-scale on which flux perturbations grow, and the time-scale on which flux perturbations decay back to the steady-state flux level respectively. The PSD of the driving impulses has a flat power spectrum on long time-scales. On time-scales shorter than $1$~d, the power spectrum begins to rise steeply possibly due to turbulent eddies in the accretion disk, or by the thermal motion of the accretion disk plasma. We conclude that valuable physical insights into accretion may be obtained using our method.
\end{abstract}
...
The right hand side (RHS) of the C-ARMA process of equation~\eqref{eq:CARMAIntro} can then be viewed as generating perturbations to the LHS, i.e. a C-ARMA process is a linear differential equation perturbed/driven by a random process. This driving process may be viewed as disturbances generated by MRI instabilities in the accretion disk. Section~\ref{sec:RHS} proposes a form for the disturbances driving the flux perturbations: coloured noise stochastic processes. A simple example of a C-ARMA process--the C-ARMA($2$,$1$) process--is presented in section~\ref{sec:CARMA21}.
...
\section[Driving Perturbations]{How to Describe the Driver of the Flux Perturbations?}\label{sec:RHS}
...
\begin{figure*}
    ...
    \caption{Why do AGN poorly-sampled AGN light curves look like a DRW? We plot two PSD models - the damped random walk (DRW) of \citet{Kelly09} (cyan) and a damped harmonic oscillator (orange). The vertical dashes are located at time-intervals proved by NASA's \textit{K2} mission (purple), the SDSS survey (red), and an idealized combination of the two (green). On timescales longer than $\sim 10$~d, both PSDs are identical explaining why the simpler DRW model is popular. The availability of better data in the future will necessitate more sophisticated C-ARMA models.}
...
The Kalman filter allows us to obtain Bayesian posteriors for the distribution function of the C-ARMA coefficients. These coefficients can then be used to compute physically meaningful time-scales that will yield insight into accretion physics. Application of the Kalman filter requires that the system be represented in \textit{state-space} form. Inferencing via the Kalman filter using a state-space representations has been successfully applied to control complex systems including the Apollo program for over $50$ years \citep{GrewalAndrews10}, and has since then come into vogue in fields as diverse as biostatistics \citep{Jones} and econometrics \citep{DurbinKoopman,Harvey}.
...
\end{equation}
where $\mathbfit{x}$ is the system-state vector, $W$ is the Wiener process of section~\ref{sec:RHS}, $v \sim \mathcal{N}(0,\sigma^{2}_{N})$ is the measurement noise, and $\chi$ is the (noisy) observed flux at time $t$. The matrix $\mathbfss{A}$ is known as the `state-transition matrix', while the matrix $\mathbfss{H}$ is known as the `observation matrix'. There are infinitely many ways of writing a C-ARMA process in state-space form--we shall discuss the 2 most relevant \citep{Denham74,Kailath74} because of their use in common packages for C-ARMA inferencing such as \textsc{carma\_pack} \citep{Kelly14}, \textsc{libcarma} (this work) etc... All other forms may be obtained from these `canonical' forms via similarity transforms.
...
\section[Sampling]{A Note on the Types of Sampling Patterns}\label{sec:Sampling}
...
If the IRS pattern is fairly sparse, it is computationally cheaper to recompute the transition matrix $\mathbfss{F}_{n}$ and the disturbance variance-covariance matrix $\mathbfss{Q}_{n}$ at every time step 
%with 
...
%\end{equation}
as proposed by \cite{JonesAckerson90}, \cite{Jones} and \cite{Kelly14}. The exact IRS pattern for which the two methods are equally fast depends on the machine architecture and hence our implementation of the C-ARMA modelling algorithm in \textsc{libcarma} allows for both methods to be used at different time-steps within the same light curve for optimal speed i.e. long gaps or sections can be treated as unevenly sampled data while occasional gaps are treated as missing values.
...
\section[Light Curve Smoothing]{What was the Light Curve Actually Doing?: Light Curve Smoothing}\label{sec:Smoothing}
...
\begin{figure*}
    \includegraphics[width=\textwidth]{images/Zw229-15_GFunc.jpg}
