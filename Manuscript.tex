...
Let the instantaneous flux emitted by an AGN accretion disk be $F(t)$. We may write $F(t) = F_{0} + \chi(t)$ where the $\chi)t)$ are perturbations to the mean flux, $F_{0}$. While the physical origins of AGN variability remain unclear, the phenomenon is known to be non-linear in the X-ray \citep{UMV05} suggesting that some non-linear equation(s) governs the behavior of $F(t)$. It is not necessarily the case that this equation is physically meaningful i.e. it may be purely phenomenological, arising out of the complex interactions of numerous physical phenomenon. Regardless, if the $\chi(t)$ are small (e.g. $\sim 10 \%$ in the optical), we can \textit{linearize} the non-linear equation that governs $F(t)$ and obtain a \textit{linear} equation for the perturbations $\chi(t)$. In appendix~\ref{sec:LHS}, we show how the linearization of an arbitrary non-linear equation for the flux $F(t)$ yields the left hand side (LHS) of the C-ARMA equation,
...
The Green's function of equation~\eqref{eq:LHS} can provide insights into how accretion physics works. Conceptually, the Green's function of a linear system characterizes the impulse response of the system. Particular solutions are found by linear superposition (integration) of the Green's function over all the impulses that drive the system \citep{PanditWu}. Here, the Green's function quantifies how a unit flux perturbation evolves after it is triggered. By comparing the Green's function to the expected behaviour of perturbations, we can gain insight into accretion physics. The form of the Green's function for equation~\eqref{eq:LHS} is given by
...
\end{equation}
with $q < p$. This form for the input stochastic impulses can be characterized by the power spectral density (PSD) of the impulses given by
